...
# RRAMC base, and recovery registration path.
#
# nRF54LM20A/B target for pyOCD - self-contained, works on pyOCD 0.42+
...
FLASH_ALGO = {
    ...
    'begin_stack' : 0x20000300, 'begin_data' : 0x20000000 + 0x1000,
    'page_size' : 0x4, 'analyzer_supported' : False, 'analyzer_address' : 0x00000000,
    'page_buffers' : [0x20001000, 0x20001004], 'min_program_length' : 0x4,
    'flash_start': 0x0, 'flash_size': 0x1FD000,
    'sector_sizes': ((0x0, 0x1FD000),)
}

class NRF54LM20A(NRF54L):
    MEMORY_MAP = MemoryMap(
        FlashRegion(start=0x0, length=0x001FD000, blocksize=0x1000, is_boot_memory=True, algo=FLASH_ALGO),
        FlashRegion(start=0x00FFD000, length=0x1000, blocksize=0x4, is_testable=False, is_erasable=False, algo=FLASH_ALGO),
