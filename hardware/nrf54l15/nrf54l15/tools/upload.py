...
def upload_openocd(hex_path: str, openocd_script: str, openocd_speed: int) -> int:
    if not tool_available("openocd"):
        print("ERROR: openocd is not installed or not available in PATH", file=sys.stderr)
        return 4
...
def main() -> int:
    ...
    if runner == "pyocd":
        ...
        rc = upload_openocd(args.hex, args.openocd_script, args.openocd_speed)
