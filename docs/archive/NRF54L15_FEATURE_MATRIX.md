...
| [x] | Dynamic 16-bit custom GATT | Implemented, including long reads, ATT Read Multiple Variable length/value tuple responses, contiguous queued prepare/execute writes for bounded custom characteristic values and writable `0x2901` User Description descriptors, bonded CCCD restore for matching peers, Bluefruit `setPermission()` read/write enforcement with encrypted-only versus MITM/authenticated checks, and read-only Bluefruit `setPresentationFormatDescriptor()` / `setReportRefDescriptor()` descriptors. `Bluefruit52Lib > Diagnostics > gatt_edge_cases` exposes these paths for manual scanner regression tests. | 128-bit dynamic service editing, broader descriptor host-app interop stress, and multi-service mutation remain open. |
...
| [x] | Disconnect reason reporting | Common HCI-style disconnect reason values are exposed to Bluefruit callbacks and through `Bluefruit.getLastDisconnectReason()` / `Bluefruit.disconnectReasonName()` for post-drop diagnostics. | More host/phone disconnect cause mapping and controller edge-case validation. |
...
| [ ] | Discovery | Structured staged DNS-SD/SRP records, publish/unpublish lifecycle diagnostics, Matter-stage OpenThread SRP client/ECDSA enablement, commissionable `_matterc._udp` SRP queueing, and SRP unregister-pending/registered-state reporting now exist. Infrastructure mDNS/DNS-SD and operational `_matter._tcp` registration are still incomplete. | Validate SRP against a real Thread border router/SRP server, implement infrastructure DNSSD, add operational registration, and prove discovery from a commissioner. |
