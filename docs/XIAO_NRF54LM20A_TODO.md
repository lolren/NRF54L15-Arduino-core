...
- [ ] Add a proper nPM1300 PMIC API for the internal P1.17/P1.18 I2C bus after validating the register map on hardware. The variant already exposes `PIN_PMIC_SCL` and `PIN_PMIC_SDA`.
- [ ] Add an LM20A PMIC example once the API exists. It should read PMIC status without keeping the I2C peripheral or PMIC rails active longer than needed.
